...
function gate(::GateName"u2", ::SiteType"vQubit", s::Index; cargs)
    ϕ::Real = cargs[1]
    λ::Real = cargs[2]
    return adjointmap_itensor("U", s; θ=pi / 2, ϕ=ϕ, λ=λ)
end

function gate(::GateName"u3", ::SiteType"vQubit", s::Index; cargs)
    θ::Real = cargs[1]
    ϕ::Real = cargs[2]
    λ::Real = cargs[3]
...
function gate(::GateName"u", st::SiteType"vQubit", s::Index; cargs)
...
function gate(::GateName"p", ::SiteType"vQubit", s::Index; cargs)
    ϕ::Real = cargs[1]
    return adjointmap_itensor("Phase", s; ϕ=ϕ)
end

function gate(::GateName"cp", ::SiteType"vQubit", control::Index, target::Index; cargs)
    ϕ::Real = cargs[1]
...
function gate(::GateName"rx", ::SiteType"vQubit", s::Index; cargs)
    θ::Real = cargs[1]
    return adjointmap_itensor("Rx", s; θ=θ)
end

function gate(::GateName"ry", ::SiteType"vQubit", s::Index; cargs)
    θ::Real = cargs[1]
    return adjointmap_itensor("Ry", s; θ=θ)
end

function gate(::GateName"rz", ::SiteType"vQubit", s::Index; cargs)
    θ::Real = cargs[1]
...
function gate(::GateName"crx", ::SiteType"vQubit", control::Index, target::Index; cargs)
    θ::Real = cargs[1]
    return adjointmap_itensor("CRx", control, target; θ=θ)
end

function gate(::GateName"cry", ::SiteType"vQubit", control::Index, target::Index; cargs)
    θ::Real = cargs[1]
    return adjointmap_itensor("CRy", control, target; θ=θ)
end

function gate(::GateName"crz", ::SiteType"vQubit", control::Index, target::Index; cargs)
    θ::Real = cargs[1]
    return adjointmap_itensor("CRz", control, target; θ=θ)
end

function gate(::GateName"cu1", ::SiteType"vQubit", control::Index, target::Index; cargs)
    λ::Real = cargs[1]
    return adjointmap_itensor("CU1", control, target; λ=λ)
end

function gate(::GateName"cu3", ::SiteType"vQubit", control::Index, target::Index; cargs)
    θ::Real = cargs[1]
    ϕ::Real = cargs[2]
    λ::Real = cargs[3]
