...
"""
    noiselayer(sites::Vector{<:Index}, ptm_generator_vec, ptm_generator_mat; maxbonddim)
...
function noiselayer(
    ...
    if !isnothing(maxbonddim) && maxbonddim > 0
        noise = truncate(noise; maxdim=maxbonddim)
    end
    return noise
end

"""
    noiselayer(sites::Vector{<:Index}, model::SPLNoiseModel; maxbonddim)
...
function noiselayer(sites::Vector{<:Index}, model::SPLNoiseModel; maxbonddim=nothing)
    return noiselayer(sites, noise_ptm_generators(model)...; maxbonddim=maxbonddim)
end
